...
def calculate(fileName):
    ...
    #Filho 1, se for um lugar possivel de ir e ele nao esta presente na fila, calculamos a distancia entre ele e o final do labirinto, e a distancia entre ele 
    #e o inicio do labirinto, somamos essas duas distancias e colocamos ela no vetor de distancias
    #Ao fim, adicionamos esse filho a fila
    #Caso esse filho seja possivel, colocamos ele como uma visita e o valor atual sera seu anterior
        if  fila[0][0] - 1 >= 0 and (labirinto[fila[0][0] - 1][fila[0][1]] == '*' or labirinto[fila[0][0] - 1][fila[0][1]] == '$') and ([fila[0][0] - 1,fila[0][1]] in fila) == 0:
            ...
            distancia = (pow(pow(fila[0][0] -1 - fim[0],2) + pow(fila[0][1] - fim[1],2),1/2)    +  pow(pow(fila[0][0] -1 - inicio[0],2) + pow(fila[0][1] - inicio[1],2),1/2))
    ...
    #Filho 2
        if  fila[0][1] - 1 >= 0 and (labirinto[fila[0][0]][fila[0][1]-1] == '*' or labirinto[fila[0][0]][fila[0][1]-1] == '$') and ([fila[0][0],fila[0][1]-1] in fila) == 0:
            ...
            distancia = (pow(pow(fila[0][0]- fim[0],2) + pow(fila[0][1] - 1 - fim[1],2),1/2)   +   pow(pow(fila[0][0]- inicio[0],2) + pow(fila[0][1] - 1 - inicio[1],2),1/2))
            dist = np.insert(dist,len(fila)-1,distancia)
